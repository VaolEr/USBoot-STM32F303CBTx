...
#define BOOTLOADER_KEY_START_ADDRESS                             (uint32_t)0x08009800//(uint32_t)0x08009800
#define BOOTLOADER_KEY_PAGE_NUMBER                               19		//10 x 2KB
...
#define NUM_OF_PAGES                                             128 //for STM32F303xB/C 256 for STM32F358xC
#define FLASH_PAGE_SIZE                                          0x800 // 2KB = 2048 bytes defined in hal_flash_ex.h


// Bootloader Report Id's and flags TO HOST
#define RI_bootloader 		0xD0
#define ReadyToUpdate 		0x77
#define RequestNewSubPage	0x0C


// Bootloader Report Id's FROM HOST


//select size of page, depends from MCU
...
void Send_ReadyMessage_To_USBHost(void);
void Send_RequestNewSubPage_To_USBHost(void);
void Send_FlashErasedStatus_To_USBHost(bool FlashEraseResult);

void SetKey(void);
void ResetKey(void);
uint32_t ReadKey(void);
//void FLASH_EraseCountPages(void);
